<template>
  <div class="container-fluid text-center mt-7">
    ...
    <div class="sentiment-label">Market Sentiment: 
      <span class='text-negative' :class="{ 'text-positive': sentimentBool}">{{ sentiment }}</span></div>
    <div id="sentimentLine" class="table">
      <table>
        <tr>
          <th>Positive:</th>
          <th>Negative:</th>
...
onMounted(() => {
  ...
  const fetchNewsPromise = new Promise((resolve) => {
    ...
    // const fetchNews = async (page=1, q=selectedName) => {
    //   try {
    ...
    //     for (let i = 0; i < 5; i++) {
    //       console.log(i)
    //       console.log(newsData.value.articles[i].title)

    //       descriptionValue = descriptionValue.concat(newsData.value.articles[i].title + ' ');
    //     }
...
//   marketSentiment(selectedSymbol);
//   getComments();
// })
